...
def main(s_version, ram_amount, auto_server_backup):
    ...
    if version == "1.8.9":
        ...
        logging.info("Executing system command: java -Xmx{ram_amount}M -Xms{ram_amount}M -Dlog4j2.formatMsgNoLookups=true -jar server.jar nogui")
        ...
        logging.info("Version not supported")
        sys.exit(1)
    if auto_server_backup == "True":
        logging.info("Auto Server Backup is enabled")
        auto_backup(version)
        pass
    else:
        logging.info("Auto Server Backup is disabled")
        pass
